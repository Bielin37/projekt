...
<div class="container border rounded">
	<header class="text-center" ><h1 class="mb-4"> Witamy <?php echo $_SESSION['imie']; ?> w panelu administracyjnym</h1></header>
		<div class="container m-1">
			<div class="row">
				<div class="col-xs-12 col-sm-6 border rounded">
					<h1 class="text-uppercase page-label2 text-danger">Lista użytkowników</h1>
					<?php
						...
						try
							{
								...
								$polaczenie -> query ('SET CHARACTER_SET utf8_unicode_ci');						
													...
													for ($i = 1; $i <= $ilu_userow; $i++)
														{
															...
															$email = $wiersz['E_mail'];			
															echo '<p class="text-zam">ID klienta: '.$idKlienta.'| Imię: '.$Imie.'| Nazwisko :'.$Nazwisko.'| e-mail : '.$email;
														}	
												} 			
											else 
												{
									...
									{
										$_SESSION['e_bladserwer']= '<div class="false"><div class="alert alert-danger alert-dismissible alert_font"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Błąd !</strong> serwera. Prosimy spróbowaćpóźniej.</div></div>';
										/*echo '<br />Informacja developerska: '.$e;*/
									}		
						?>										
				</div>
				<div class="col-xs-12 col-sm-6 border rounded">
					<h1 class="text-uppercase page-label2 text-danger">Lista zamówień</h1>
					<?php
						require_once "connect.php";
						mysqli_report(MYSQLI_REPORT_STRICT);	
						try
							{
								...
								$polaczenie -> query ('SET CHARACTER_SET utf8_unicode_ci');
									if ($polaczenie->connect_errno!=0)
										...
										{				
										if ($rezultat = @$polaczenie->query(
											sprintf("SELECT * FROM Zamowienia",
											mysqli_real_escape_string($polaczenie,$idKlient))))
											{
												$ilu_userow = $rezultat->num_rows;
											if($ilu_userow>0)
